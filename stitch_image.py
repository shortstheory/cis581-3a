...
def stitch_image(img1, img2, H):
    ...
    # canvas = cv2.warpPerspective(img2, np.linalg.inv(H),(int(img1.shape[1]*2), int(img1.shape[0]*2)))
    # mosaic = canvas
    # scaled_img = mosaic
    ...
    # H[0,2] = min(0,-xmin)
    # H[1,2] = min(0,-ymin)
    # canvas = np.zeros((2000,2000,3))
    # border = 0
    # img1 = cv2.copyMakeBorder(img1, border,border,border,border,cv2.BORDER_CONSTANT)

    # canvas[500:img1.shape[0]+500,500:500+img1.shape[1]] = img1
    canvas = cv2.warpPerspective(img1, T@H,(int(img1.shape[1]*2),int(img1.shape[0]*2)))
    # canvas[border:border+img2.shape[0],border:border+img2.shape[1]]=img2
    _x = abs(max(0,-xmin))
    _y = abs(max(0,-ymin))
    canvas[_y:_y+img2.shape[0],_x:_x+img2.shape[1]]=img2
    mosaic = canvas
    scaled_img = mosaic
